    ...
    <div className="max-w-3xl mx-auto px-3 sm:px-4 space-y-6 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Company & national holidays</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="glass-card p-8 text-center">
          ...
          <button onClick={() => setShowForm(true)} className="btn-primary btn-sm mt-3">Add First Holiday</button>
        </div>
      ) : (
                ...
                {grouped[m].map(h => {
                  ...
                  return (
                    <motion.div key={h._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      className={`flex items-center gap-3 p-3 rounded-xl border ${today ? 'bg-green-50 border-green-200' : past ? 'bg-gray-50 border-gray-100 opacity-60' : 'bg-white border-gray-100'} shadow-sm`}>
                      <div className={`w-11 h-11 rounded-xl flex flex-col items-center justify-center flex-shrink-0 text-center ${today ? 'bg-green-500 text-white' : 'bg-violet-50 text-violet-800 border border-violet-100'}`}>
                      ...
                      <button onClick={() => handleDelete(h._id)}
                        className="text-xs font-semibold text-red-400 hover:text-red-600 flex-shrink-0 ml-1">
                        ✕
                      </button>
                    </motion.div>
                  );
