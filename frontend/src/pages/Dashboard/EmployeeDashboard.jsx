...
// import { formatCurrency, getInitials, monthName } from '../../utils/helpers';
...
// const fade = (i = 0) => ({ hidden: { opacity: 0, y: 14 }, visible: { opacity: 1, y: 0, transition: { delay: i * 0.06, duration: 0.28 } } });

// const QuickLink = ({ to, bg, text, icon, label }) => (
//   <Link to={to}
//     className={`${bg} ${text} rounded-xl flex flex-col items-center justify-center gap-2 p-3 min-h-[80px] hover:opacity-85 active:scale-95 transition-all duration-150`}>
//     <Icon d={icon} className="w-6 h-6 flex-shrink-0" />
...
//   <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-violet-50">
//     <h3 className="font-bold text-violet-900 text-sm">{title}</h3>
//     {linkTo && <Link to={linkTo} className="text-xs font-semibold text-amber-600 hover:text-amber-700">{linkLabel} →</Link>}
//   </div>
// );
...
//     const reqs = [
//       api.get(`/attendance/my?month=${m}&year=${y}`).then(r => { setAttendance(r.data.summary); setTodayRecord(r.data.todayRecord); }).catch(() => {}),
...
//       });
//       await api.post('/attendance/check-in', { lat: pos.coords.latitude, lng: pos.coords.longitude });
//       toast.success('Checked in!');
//       fetchDashboard();
//     } catch (err) {
//       const msgs = { 1: 'Location access denied.', 2: 'Location unavailable.', 3: 'Location timed out.' };
//       toast.error(msgs[err.code] || err.response?.data?.message || err.message || 'Check-in failed');
//     } finally { setCheckLoading(false); }
//   };

//   const handleCheckOut = async () => {
//     setCheckLoading(true);
//     try { await api.post('/attendance/check-out'); toast.success('Checked out!'); fetchDashboard(); }
...
//       <Spinner size="lg" /><p className="text-sm text-violet-500 font-medium">Loading dashboard…</p>
//     </div>
//   );
...
//         {/* top row: avatar + name */}
//         <div className="relative px-4 pt-4 pb-3 flex items-center gap-3 min-h-[64px] sm:min-h-[72px] lg:min-h-[84px]">
//           <div className="w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 rounded-2xl bg-white/20 flex items-center justify-center text-lg sm:text-xl font-bold flex-shrink-0 ring-2 ring-white/20 banner-avatar">
//             {getInitials(user?.name)}
//           </div>
//           <div className="min-w-0 flex-1 flex flex-col justify-center">
//             <p className="text-violet-300 text-xs sm:text-sm leading-tight mb-0.5">{greeting()},</p>
//             <h2 className="text-lg sm:text-2xl font-bold truncate leading-tight">{user?.name}</h2>
//             <p className="text-violet-300 text-[11px] sm:text-sm truncate mt-0.5">
...
//         <div className="relative px-4 py-3 flex flex-wrap items-center gap-2">
//           {/* status pill */}
...
//             <span className={`w-1.5 h-1.5 rounded-full ${todayRecord?.checkIn ? 'bg-green-300' : 'bg-violet-400'}`} />
//             {todayRecord?.checkIn
...
//                 className="px-3 py-1 bg-green-500 hover:bg-green-400 disabled:opacity-50 rounded-lg text-xs font-bold text-white transition-colors">
//                 {checkLoading ? '…' : 'Check In'}
//               </button>
//             )}
//             {todayRecord?.checkIn && !todayRecord?.checkOut && (
//               <button onClick={() => setConfirmCheckout(true)} disabled={checkLoading}
//                 className="px-3 py-1 bg-red-500 hover:bg-red-400 disabled:opacity-50 rounded-lg text-xs font-bold text-white transition-colors">
//                 Check Out
//               </button>
//             )}
//             {todayRecord?.checkIn && todayRecord?.checkOut && (
//               <span className="px-2.5 py-1 bg-green-400/20 text-green-200 rounded-lg text-xs font-semibold">✓ Day Complete</span>
//             )}
...
//               <div className="relative px-4 py-4 flex items-center justify-between gap-4">
//                 <div className="flex items-center gap-4 flex-1 min-w-0">
//                   <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 lg:w-32 lg:h-32 flex-shrink-0 profile-circle-banner">
...
//                           <div className="text-sm sm:text-base font-bold text-white leading-tight">{profileCompletion.percentage}%</div>
//                           <div className="text-[10px] sm:text-[11px] text-white/85">Complete</div>
//                         </div>
//                       </div>
//                     </div>
//                   </div>
//                   <div className="min-w-0 flex-1">
//                     <h3 className="font-bold text-base sm:text-lg leading-tight">Complete Your Profile</h3>
//                     <p className="text-xs sm:text-sm text-white/90 mt-0.5">
//                       Fill in {100 - profileCompletion.percentage}% more details and upload your photo to unlock payslip downloads
//                     </p>
//                   </div>
//                 </div>
...
//           <div className={`p-3 grid gap-2 ${isSales ? 'grid-cols-3 sm:grid-cols-5' : 'grid-cols-2 sm:grid-cols-4'}`}>
...
//             <SectionHeader title="Leave Balance" linkTo="/leaves" linkLabel="Apply / View all" />
...
//                         <span className="text-sm font-medium text-violet-800 capitalize">{type}</span>
//                         <span className="text-xs text-violet-400">{bal.used}/{bal.total} used</span>
//                       </div>
...
//                       </div>
//                       <p className="text-xs text-right text-green-600 font-semibold mt-0.5">{bal.remaining} remaining</p>
//                     </div>
//                   ))}
//                 </div>
//               ) : (
//                 <div className="flex flex-col items-center justify-center h-full py-6 text-violet-300 gap-2">
//                   <Icon d={IC.clock} className="w-8 h-8" />
//                   <p className="text-sm text-violet-400">No leave data</p>
//                   <Link to="/leaves" className="text-xs font-semibold text-amber-600 hover:text-amber-700">Apply Leave →</Link>
//                 </div>
//               )}
//             </div>
//           </SectionCard>
...
//                 <div className="space-y-3">
//                   {/* Net pay highlight */}
//                   <div className="bg-gradient-to-r from-violet-50 to-amber-50 rounded-xl p-3 flex items-center justify-between">
//                     <div>
//                       <p className="text-sm font-bold text-violet-900">{monthName(payslip.month)} {payslip.year}</p>
//                       <p className="text-xs text-violet-400 mt-0.5">Net Pay</p>
//                     </div>
//                     <div className="text-right">
//                       <p className="text-xl font-extrabold text-amber-600 leading-tight">{formatCurrency(payslip.netSalary)}</p>
...
//                       <p className="text-sm font-bold text-red-600">{formatCurrency(require('../../utils/helpers').sumMoney(payslip.deductions) ?? 0)}</p>
//                     </div>
//                   </div>
//                 </div>
//               ) : (
//                 <div className="flex flex-col items-center justify-center h-full py-6 text-violet-300 gap-2">
//                   <Icon d={IC.card} className="w-8 h-8" />
//                   <p className="text-sm text-violet-400">No payslips yet</p>
//                   <Link to="/payslips" className="text-xs font-semibold text-amber-600 hover:text-amber-700">View Payslips →</Link>
//                 </div>
//               )}
//             </div>
//           </SectionCard>
...
//         <SectionCard>
//           <button className="w-full flex items-center justify-between px-4 py-3 text-left" onClick={() => setShowTeam(v => !v)}>
//             <div className="flex items-center gap-2 min-w-0">
//               <Icon d={IC.users} className="w-4 h-4 text-violet-600 flex-shrink-0" />
//               <span className="font-bold text-violet-900 text-sm">My Team</span>
//               {teamDept && <span className="text-xs text-violet-400 truncate">· {teamDept}</span>}
//               {team.length > 0 && (
//                 <span className="flex-shrink-0 text-[10px] bg-violet-100 text-violet-700 font-bold px-1.5 py-0.5 rounded-full">{team.length}</span>
//               )}
//             </div>
//             <Icon d={IC.chevron} className={`w-4 h-4 text-violet-400 flex-shrink-0 transition-transform duration-200 ${showTeam ? 'rotate-180' : ''}`} />
//           </button>

//           {showTeam && (
//             <div className="border-t border-violet-50 p-3">
//               {team.length === 0 ? (
//                 <p className="text-center text-sm text-violet-400 py-4">
//                   {teamDept ? `No other members in ${teamDept} yet.` : 'No department assigned. Ask HR to assign yours.'}
//                 </p>
//               ) : (
...
//                       <div className="w-9 h-9 rounded-full bg-gradient-to-br from-violet-500 to-violet-700 text-white flex items-center justify-center text-sm font-bold flex-shrink-0">
//                         {getInitials(m.name)}
//                       </div>
//                       <div className="min-w-0">
//                         <p className="text-sm font-semibold text-violet-900 truncate">{m.name}</p>
//                         <p className="text-[11px] text-violet-500 truncate capitalize">{m.designation || m.role}</p>
//                         <p className="text-[10px] text-violet-400">{m.employeeId}</p>
//                       </div>
//                     </div>
//                   ))}
//                 </div>
...
//             className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm border border-red-100">
//             <div className="flex items-center justify-center w-12 h-12 rounded-full bg-red-100 mx-auto mb-4">
//               <Icon d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" className="w-6 h-6 text-red-600" />
//             </div>
//             <h3 className="text-center font-bold text-violet-900 text-lg mb-1">Confirm Check-Out</h3>
//             <p className="text-center text-sm text-violet-500 mb-6">
//               Are you sure you want to check out now?<br />This will record your departure time.
//             </p>
//             <div className="flex gap-3">
//               <button onClick={() => setConfirmCheckout(false)}
//                 className="flex-1 px-4 py-2 rounded-xl border border-violet-200 text-sm font-semibold text-violet-700 hover:bg-violet-50 transition-colors">
//                 Cancel
//               </button>
//               <button onClick={() => { setConfirmCheckout(false); handleCheckOut(); }} disabled={checkLoading}
//                 className="flex-1 px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 disabled:opacity-50 text-sm font-bold text-white transition-colors">
//                 {checkLoading ? 'Checking out…' : 'Yes, Check Out'}
//               </button>
//             </div>
//           </motion.div>
//         </div>
...
//                     { label: 'Conversion %', value: `${crmStats.conversionRate}%`, icon: <Icon d={IC.trend} />, color: 'golden', to: '/crm' },
//                   ].map(c => <KpiCard key={c.label} {...c} />)}
//                 </div>
//               ) : (
//                 <div className="flex flex-col items-center gap-2 py-6 text-violet-300">
//                   <Icon d={IC.crm} className="w-8 h-8" />
//                   <p className="text-sm text-violet-400">No CRM data yet</p>
//                   <Link to="/crm" className="text-xs font-semibold text-amber-600 hover:text-amber-700">Add Leads →</Link>
//                 </div>
//               )}
//             </div>
//           </SectionCard>
